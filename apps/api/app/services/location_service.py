...
def _is_matching_state(expected_state: str | None, candidate_state: str | None) -> bool:
    if not expected_state:
        return True

    return _normalize_text(expected_state) == _normalize_text(candidate_state)


def _is_matching_country(
    ...
    return _normalize_text(expected_country) == _normalize_text(candidate_country)
...
def _parse_country_items(payload: dict[str, Any]) -> list[CountryOption]:
...
async def get_countries() -> list[CountryOption]:
    primary_payload = await _fetch_json("GET", f"{COUNTRIESNOW_BASE_URL}/positions")
    countries = _parse_country_items(primary_payload)
    if countries:
        return countries

    fallback_payload = await _fetch_json("GET", f"{COUNTRIESNOW_BASE_URL}/states")
    countries = _parse_country_items(fallback_payload)
    if countries:
        return countries
...
def _extract_state_items(payload: dict[str, Any]) -> list[dict[str, Any]]:
...
def _normalize_state_option(item: dict[str, Any]) -> StateOption | None:
...
async def get_states(country: str) -> list[StateOption]:
    ...
    states = [
        state
        for item in _extract_state_items(payload)
        if (state := _normalize_state_option(item)) is not None
    ]
...
def _display_label(city: str, state: str, country: str) -> str:
    parts = [part for part in [city, state, country] if part]
    return ", ".join(parts)


def _normalize_city_result(
    item: dict[str, Any],
    ...
    if not _is_matching_state(selected_state, state):
        return None
    ...
    return CitySuggestion(
        ...
        latitude=latitude,
        longitude=longitude,
        display_label=_display_label(city.strip(), state.strip(), country.strip()),
    )
...
async def search_cities(
    ...
    payload = await _fetch_json("GET", OPEN_METEO_GEOCODING_URL, params=params)
    ...
    for item in raw_results:
        ...
        suggestion = _normalize_city_result(
            item,
    ...
    return suggestions[:MAX_CITY_RESULTS]
